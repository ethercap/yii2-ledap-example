ledap.App.register(["form-item","detail", "group", "tab", 'grid', 'pager', "baseinput"], Vue);
const app = new Vue({
  el: "#app",
  data : {
    dp : ledap.App.getWebDp({
        httpOptions:{
            url: "/ledap/score/index",
            params: GetParams(),
        },
    }),
    model: ledap.App.getModel(data.model),
    isLoading: false,
    modalConfig:{
        show:false,
        type:"view",
    },
    columns : [
        'id',
        'lessonId',
        'studentId',
        'score',
        'creationTime',
        //'updateTime',
        {
            'label' : '操作',
            'value' : function(model) {
                return `<div class="btn-group">
                    <a class="btn btn-primary text-white" @click="vm.view(model)" >查看</a>
                    <a class="btn btn-warning text-white" @click="vm.update(model)">编辑</a>
                    <a class="btn btn-danger text-white" @click="vm.remove(model)">删除</a></div>`; 
            },
            'format' : 'html',
        }
    ],
    detailColumns : [
        'id',
        'lessonId',
        'studentId',
        'score',
        'creationTime',
        //'updateTime',
    ],
    
  },
  created: function(){
    this.dp.isLoading = true;
    this.dp.refresh("");
  },
  methods : {
    refresh: function(){
        this.dp.refresh("");
    },
    remove: function(model){
        if(confirm("你确定要删除该数据")) {
            ledap.App.request({
                url: "/ledap/score/delete?id=" + model.id,
                method: 'POST',
            }, function() {
                this.dp.remove(model);
            }.bind(this))
        }
    },
    create: function() {
        this.model = ledap.App.getModel(data.model);
        this.modalConfig = {
            show: true,
            type: "create",
        };
    },
    update: function(model){
        this.getRemoteModel(model);
        this.modalConfig = {
            show: true,
            type: "update",
        };
    },
    view: function(model) {
        this.getRemoteModel(model);
        this.modalConfig = {
            show:true,
            type: "view",
        };
    },
    //如果与table字段一致，可以直接this.model = model;
    getRemoteModel: function(model){
        this.isLoading = true;
        let url = '/ledap/score/view?id='+model.id;
        ledap.App.request({
            url: url,
            method: 'GET',
        }, function(res){
            this.isLoading = false;
            this.model = ledap.App.getModel(data.model).load(res.data);
        }.bind(this), function(res) {
            this.isLoading = false;
            this.$toast(res.message, {variant:'danger'});
        }.bind(this));
    },
    submit: function() {
        event.preventDefault();
        if(!this.model.validate()) {
            this.$toast(this.model.getFirstError(), {variant:'warning'});
            return false;
        }
        let url = this.modalConfig.type === "create" ? '/ledap/score/create' : '/ledap/score/update?id='+this.model.id;
        this.isLoading = true;
        ledap.App.request({
            url: url,
            method: 'POST',
            data: this.model
        }, function(res) {
            this.model.load(res.data);
            this.isLoading  = false;
            if(!this.model.hasErrors()) {
                this.$toast("操作成功");
                this.modalConfig.show = false;
                this.refresh();
            }
        }.bind(this), function(res){
            this.isLoading = false;
            this.$toast(res.message, {variant:'danger'});
        }.bind(this));
    }
  }
});

